#ifndef ASYNC_ZK_ZKCLIENT_H_
#define ASYNC_ZK_ZKCLIENT_H_
#include <string>
#include <deque>
#include <set>
...
namespace async_zk {
  ...
  /**
   * callable that keeps trying to get task from task queue and execute task
   * if task finishes successfully, deque it
   * else put it in queue and try again
   */ 
  ...
  TaskRunner(TaskQueue<ZKTaskWrapper>* queue)
    ...
    void operator()() {
      while (!stop_) {
	std::pair<ZKTaskWrapper, bool> result = queue_->pop_front();
	if (!result.second) {
	  continue;
	}
	ZKTaskWrapper& task_wrapper = result.first;
	LOG(INFO)<< "task_type: " << ZKTaskWrapper::task_type(task_wrapper);
	int rc = task_wrapper();
	switch (rc) {
        case ZOK:
          break;
      ...
      tasks_.push_back(
		       ZKTaskWrapper(
				     boost::bind(&ZKClient::add_expired_listener_task, this, path,
						 listener),
				     ZKTaskWrapper::ADD_LISTENER_TASK));
    }
    void add_auth(const std::string& user, const std::string& passwd) {
      tasks_.push_back(
		       ZKTaskWrapper(
				     boost::bind(&ZKClient::add_auth_task, this, user, passwd), ZKTaskWrapper::ADD_AUTH_TASK));
    }
